...
class V44__RenameStatusesTest extends UnitSuite with TestEnvironment {
  ...
  test("That converting content-links works as expected") {
    ...
    val expectedResult =
      """{"content":[{"content":"<section></section>","language":"nb"}],"status":{"current":"PLANNED","other":["IN_PROGRESS","EXTERNAL_REVIEW","QUALITY_ASSURANCE","LANGUAGE","FOR_APPROVAL","END_CONTROL","END_CONTROL","END_CONTROL","PUBLISH_DELAYED","PUBLISHED","PUBLISHED"]}}"""
