...
class V16__AddProcessedCopyrightField extends BaseJavaMigration {
  def countAllRows(implicit session: DBSession): Option[Long] = {
    sql"select count(*) from imagemetadata where metadata is not NULL"
      .map(rs => rs.long("count"))
      .single()
  }

  def allRows(offset: Long)(implicit session: DBSession): Seq[(Long, String)] = {
    sql"select id, metadata from imagemetadata where metadata is not null order by id limit 1000 offset $offset"
      .map(rs => {
  ...
  def updateRow(document: String, id: Long)(implicit session: DBSession): Int = {
    ...
    sql"update imagemetadata set metadata = $dataObject where id = $id"
      .update()
  }
