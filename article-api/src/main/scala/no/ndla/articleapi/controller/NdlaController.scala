...
import no.ndla.articleapi.{ComponentRegistry, WithProps}
...
trait NdlaController {
  this: WithProps with ErrorHelper =>

  abstract class NdlaController(connectToDatabase: () => Unit)
    ...
    after() {
      CorrelationID.clear()
      ThreadContext.remove(props.CorrelationIdKey)
      AuthUser.clear()
    ...
    error {
      case a: AccessDeniedException if a.unauthorized =>
        Unauthorized(body = Error(ErrorHelper.ACCESS_DENIED, a.getMessage))
      case a: AccessDeniedException => Forbidden(body = Error(ErrorHelper.ACCESS_DENIED, a.getMessage))
      case v: ValidationException =>
        BadRequest(
          body = ValidationError(ErrorHelper.VALIDATION, ErrorHelper.VALIDATION_DESCRIPTION, messages = v.errors))
      case _: IndexNotFoundException                    => InternalServerError(body = ErrorHelper.IndexMissingError)
      case NotFoundException(message, sl) if sl.isEmpty => NotFound(body = Error(ErrorHelper.NOT_FOUND, message))
      case NotFoundException(message, supportedLanguages) =>
        NotFound(body = Error(ErrorHelper.NOT_FOUND, message, supportedLanguages = Some(supportedLanguages)))
      case rw: ResultWindowTooLargeException =>
        UnprocessableEntity(body = Error(ErrorHelper.WINDOW_TOO_LARGE, rw.getMessage))
      case _: PSQLException =>
        connectToDatabase()
        InternalServerError(Error(ErrorHelper.DATABASE_UNAVAILABLE, ErrorHelper.DATABASE_UNAVAILABLE_DESCRIPTION))
      case NdlaSearchException(_, Some(rf), _)
        ...
        BadRequest(body = ErrorHelper.InvalidSearchContext)
      case t: Throwable =>
        logger.error(ErrorHelper.GenericError.toString, t)
        InternalServerError(body = ErrorHelper.GenericError)
    }
