...
class InternControllerTest extends UnitSuite with ScalatraSuite with TestEnvironment {
  ...
  val DefaultApiImageMetaInformation = api.AudioMetaInformation(1, 1, api.Title("title", "nb"), api.Audio("audio/test.mp3", "audio/mpeg", 1024, "nb"), api.Copyright(api.License("by-sa", None, None), None, Seq(), Seq(), Seq(), None, None, None), api.Tag(Seq("tag"), "nb"), Seq("nb") )
  val DefaultDomainImageMetaInformation = domain.AudioMetaInformation(Some(1), Some(1), Seq(domain.Title("title", "nb")), Seq(domain.Audio("audio/test.mp3", "audio/mpeg", 1024, "nb")), domain.Copyright("by-sa", None, Seq(), Seq(), Seq(), None, None, None), Seq(domain.Tag(Seq("tag"), "nb")), "ndla124", updated)
  val DefaultDomainAudioNoLanguage = domain.AudioMetaInformation(Some(1), Some(1), Seq(domain.Title("title", "unknown")), Seq(domain.Audio("audio/test.mp3", "audio/mpeg", 1024, "unknown")), domain.Copyright("by-sa", None, Seq(), Seq(), Seq(), None, None, None), Seq(domain.Tag(Seq("tag"), "unknown")), "ndla124", updated)
