...
lazy val `article-api` = Module.setup(
  ...
  articleapi,
  deps = Seq(network, mapping, language, validation, scalatestsuite % "test", common, search)
)
...
lazy val `audio-api` = Module.setup(
  ...
  audioapi,
  deps = Seq(network, mapping, language, scalatestsuite % "test", common, search)
)
...
lazy val `frontpage-api` = Module.setup(
  ...
  frontpageapi,
  deps = Seq(network, mapping, language, scalatestsuite % "test", common)
)
