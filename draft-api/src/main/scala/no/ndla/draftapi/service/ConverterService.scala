...
trait ConverterService {
  ...
  class ConverterService extends LazyLogging {
    ...
    def getEmbeddedConceptIds(article: common.draft.Article): Seq[Long] = {
    ...
    def getEmbeddedH5PPaths(article: common.draft.Article): Seq[String] = {
    ...
    def updateStatus(
        ...
        user: UserInfo,
        isImported: Boolean
    ): IO[Try[common.draft.Article]] = StateTransitionRules.doTransition(article, status, user, isImported)

    def toApiArticle(article: common.draft.Article, language: String, fallback: Boolean = false): Try[api.Article] = {
    ...
    def getNextRevision(article: common.draft.Article): Option[common.draft.RevisionMeta] = getNextRevision(
    ...
    def toArticleApiArticle(article: common.draft.Article): api.ArticleApiArticle = {
    ...
    private def getNewEditorialNotes(
        ...
        toMergeInto: common.draft.Article
    ): Try[Seq[common.EditorNote]] = {
    ...
    private[service] def _stateTransitionsToApi(
        user: UserInfo,
        article: Option[common.draft.Article]
    ...
    def addNote(article: common.draft.Article, noteText: String, user: UserInfo): common.draft.Article = {
      article.copy(
