...
import _root_.io.searchbox.client.{JestClient, JestResult}
...
object JestClientFactory {
  def getClient(searchServer: String = AudioApiProperties.SearchServer): NdlaJestClient = {
  ...
  private def getNonSigningClient(searchServer: String): NdlaJestClient = {
    ...
    factory.setHttpClientConfig(new HttpClientConfig.Builder(searchServer).multiThreaded(true).build())
