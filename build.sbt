...
lazy val `frontpage-api`    = Module.setup(project in file("./frontpage-api/"),    frontpageapi,      deps = Seq(network, mapping, language,             scalatestsuite % "test", common))
...
lazy val integration = Module.setup(project in file("./integration-tests/"), integrationtests, deps = Seq(scalatestsuite, `draft-api`, `article-api`))
