...
trait IndexService {
  ...
  trait IndexService extends BaseIndexService with StrictLogging {
    ...
    override def getNewIndexName(): String = s"${searchIndex}_$getTimestamp"
    ...
    def indexDocumentsInBulk(
        numShards: Option[Int]
    )(sendToElasticFunction: SendToElastic): Try[ReindexResult] =
      for {
    ...
    def updateReplicaNumber(ReplicaNumber: Int): Try[?] = getAliasTarget.flatMap {
      case None            => Success(())
      case Some(indexName) =>
        updateReplicaNumber(indexName, ReplicaNumber.some)
    }

    private def updateReplicaNumber(indexName: String, ReplicaNumber: Option[Int]): Try[?] = {
      if (props.Environment == "local") {
        logger.info("Skipping replica change in local environment, since the cluster only has one node.")
        Success(())
      } else {
        val updateValue = ReplicaNumber.getOrElse(indexReplicas).toString
      ...
      if (!indexWithNameExists(newIndexName).getOrElse(false)) {
        ...
        e4sClient.execute(aliases(actions)) match {
          case Success(_) =>
            ...
            for {
              _ <- cleanupIndexes()
              _ <- updateReplicaNumber(newIndexName, ReplicaNumber = None)
            } yield ()
          case Failure(ex) =>
