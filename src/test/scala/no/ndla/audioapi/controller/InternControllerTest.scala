...
class InternControllerTest extends UnitSuite with ScalatraSuite with TestEnvironment {
  ...
  val DefaultApiImageMetaInformation = api.AudioMetaInformation(1, "title", Seq(api.Audio("audio/test.mp3", "audio/mpeg", 1024, Some("nb"))), api.Copyright(api.License("by-sa", None, None), None, Seq()), Seq())
