...
class InternControllerTest extends UnitSuite with ScalatraSuite with TestEnvironment {
  ...
  val DefaultApiImageMetaInformation = api.AudioMetaInformation(1, 1, "", "title", api.Audio("audio/test.mp3", "audio/mpeg", 1024), api.Copyright(api.License("by-sa", None, None), None, Seq()), Seq("nb"), Seq("tag"))
  val DefaultDomainImageMetaInformation = domain.AudioMetaInformation(Some(1), Some(1), Seq(domain.Title("title", Some("nb"))), Seq(domain.Audio("audio/test.mp3", "audio/mpeg", 1024, Some("nb"))), domain.Copyright("by-sa", None, Seq()), Seq(Tag(Seq("tag"), Option("nb"))), "ndla124", updated)
  val DefaultDomainAudioNoLanguage = domain.AudioMetaInformation(Some(1), Some(1), Seq(domain.Title("title", None)), Seq(domain.Audio("audio/test.mp3", "audio/mpeg", 1024, None)), domain.Copyright("by-sa", None, Seq()), Seq(Tag(Seq("tag"), None)), "ndla124", updated)
