...
class StateTransitionRulesTest extends UnitSuite with TestEnvironment {
  ...
  test("stateTransitionsToApi should allow all users to archive articles that have not been published") {
    ...
    when(draftRepository.withId(eqTo(articleId), any)(using any)).thenReturn(Some(article))
    val Success(noTrans) =
  ...
  test("stateTransitionsToApi should filter some transitions based on publishing status") {
    ...
    when(draftRepository.withId(eqTo(articleId), any)(using any)).thenReturn(Some(unpublished))
    ...
    when(draftRepository.withId(eqTo(articleId), any)(using any)).thenReturn(Some(published))
