        ...
        case Some(query) =>
          boolQuery()
            .must(
              boolQuery()
                .should(
                  languageSpecificSearch("titles", settings.language, query, 2, settings.fallback),
                  languageSpecificSearch("descriptions", settings.language, query, 1, settings.fallback),
                  idsQuery(query)
                )
            )
        case None => boolQuery()
