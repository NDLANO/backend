...
class MultiDraftSearchServiceTest extends IntegrationSuite(EnableElasticsearchContainer = true) with TestEnvironment {
  ...
  test("That all returns all documents ordered by lastUpdated descending") {
    ...
    results.totalCount should be(expected.size)
    results.summaryResults.head.id should be(3)
  ...
  test("That all returns all documents ordered by lastUpdated ascending") {
    ...
    results.summaryResults.last.id should be(3)
  }
  ...
  test("That filtering on multiple context-types returns every selected type") {
    ...
    search.summaryResults.map(_.id) should be(Seq(1, 2, 3, 5, 6, 7, 8, 9, 10, 11, 12, 13, 15))
  ...
  test("That filtering on supportedLanguages works") {
    ...
    search2.summaryResults.map(_.id) should be(Seq(1, 1, 2, 2, 3, 3, 4, 5, 5, 6, 6, 7, 8, 9, 10, 11, 12, 13, 15, 16))
    ...
    search3.summaryResults.map(_.id) should be(Seq(1, 1, 2, 2, 3, 3, 4, 5, 6, 7, 8, 9, 11, 12, 13, 15, 16))
  }
  ...
  test("That filtering for topics returns every child learningResource") {
    ...
    search.summaryResults.map(_.id) should be(Seq(1, 1, 2, 2, 4, 9, 12))
  }
  ...
  test("That filtering by relevance id works when no subject is specified") {
    ...
    search2.summaryResults.map(_.id) should be(Seq(1, 2, 3, 4, 5, 12, 15))
    ...
    search3.summaryResults.map(_.id) should be(Seq(1, 1, 2, 2, 3, 3, 4, 5, 5, 6, 7, 8, 9, 10, 11, 12, 15))
  }
  ...
  test("That scrolling works as expected") {
    ...
    scroll10.summaryResults.map(_.id) should be(List.empty)
