...
trait WithDefaultProps extends WithProps {
...
class ArticleApiProperties extends LazyLogging {
  ...
  def oldProcessorTypes =
    List("bearbeider", "tilrettelegger", "redaksjonelt", "språklig", "ide", "sammenstiller", "korrektur")
  def processorTypes = List("processor", "facilitator", "editorial", "linguistic", "idea", "compiler", "correction")

  def oldRightsholderTypes = List("rettighetshaver", "forlag", "distributør", "leverandør")
  def rightsholderTypes = List("rightsholder", "publisher", "distributor", "supplier")
  ...
  // everything is converted. This defue defines a maximum number of times the converter runs on a node
