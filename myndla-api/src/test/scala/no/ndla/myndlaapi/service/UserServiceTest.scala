...
class UserServiceTest extends UnitTestSuite with TestEnvironment {
  ...
  test("That cleanupInactiveUsers emails all candidates when no previous cleanup exists") {
    ...
    val usersToDelete = List(
      userWithLastSeen(id = 1, feideId = "delete-1", lastSeen = now.minusDays(220))
    )
    ...
    when(userRepository.getUserNotSeenSince(any[NDLADate])(using any))
      .thenReturn(Success(usersToDelete), Success(emailCandidates))
    ...
    service.cleanupInactiveUsers() should be(
      Success(InactiveUserResultDTO(usersToDelete.size, emailCandidates.size))
    )
    ...
    verify(userRepository, times(1)).insertCleanupResult(eqTo(usersToDelete.size), eqTo(emailCandidates.size), eqTo(now))(using any)
  }
  ...
  test("That cleanupInactiveUsers only emails users who became inactive since the last successful run") {
    ...
    when(userRepository.getLastCleanup(using any))
      .thenReturn(Success(Some(InactiveUserCleanupResult(1, 0, 0, lastCleanupDate))))
    when(userRepository.getUserNotSeenSince(any[NDLADate])(using any))
      .thenReturn(Success(usersToDelete), Success(emailCandidates))
    when(userRepository.insertCleanupResult(eqTo(0), eqTo(1), eqTo(now))(using any))
      .thenReturn(Success(InactiveUserCleanupResult(2, 0, 1, now)))
