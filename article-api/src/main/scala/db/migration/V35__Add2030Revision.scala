...
class V35__Add2030Revision extends BaseJavaMigration {
  ...
  private val formats      = DefaultFormats.withLong ++ JavaTimeSerializers.all
  private val revDate      = Extraction.decompose(revisionDate)(formats)

  private[migration] def convertArticleUpdate(document: String): String = {
    val oldArticle = parse(document)

    val mergeObject = JObject("revisionDate" -> revDate)
