...
class MyNDLAAuthHelpers(using userService: UserService, tapirErrorHandling: TapirErrorHandling) {
  import tapirErrorHandling.*
  implicit class authlessEndpointFeideExtension[A, I, E, O, R](self: Endpoint[Unit, I, AllErrors, O, R]) {
    ...
    private type PartialFeideEndpoint[F[_]] = PartialServerEndpoint[MaybeFeideToken, MyNDLAUser, I, AllErrors, O, R, F]
    def requireMyNDLAUser[F[_]](requireArena: Boolean = false): PartialFeideEndpoint[F] = {
