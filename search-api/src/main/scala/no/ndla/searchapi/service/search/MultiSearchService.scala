...
trait MultiSearchService {
  ...
  class MultiSearchService extends StrictLogging with SearchService with TaxonomyFiltering {
    ...
    override val searchIndex: List[String]   = List(SearchIndexes(SearchType.Articles), SearchIndexes(SearchType.LearningPaths))
    override val indexServices: List[MultiSearchService.this.IndexService[_ <: Content]] = List(articleIndexService, learningPathIndexService)
