...
abstract class NdlaController extends ScalatraServlet with NativeJsonSupport with LazyLogging {
  ...
  def paramOrDefault(paramName: String, default: String)(implicit request: HttpServletRequest): Option[String] =
    Option(params.get(paramName).map(_.trim).filterNot(_.isEmpty()).getOrElse(default))
