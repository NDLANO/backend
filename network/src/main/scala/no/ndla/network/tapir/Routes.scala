...
class Routes(using
  ...
  private object TapirMiddleware {
    ...
    private val requestBody     = new AttributeKey[Source[Chunk[Byte]]]("requestBody")
    ...
    class after extends RequestResultEffectTransform[Identity] {
      ...
      private def addRequestBodyMDC(req: ServerRequest): Unit =
        req.attribute(requestBody).foreach { bodySource =>
          val sb = StringBuilder()
          bodySource.foreachOrError(chunk => sb.append(chunk.asStringUtf8)) match {
  ...
  def startServerAndWait(name: String, port: Int, gracefulShutdownTimeout: FiniteDuration = 30.seconds)(
    ...
    val config    = NettyConfig.default.withGracefulShutdownTimeout(gracefulShutdownTimeout)
